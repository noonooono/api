-- 接口信息
create table if not exists sonapi.`interface_info`
(
...
-- 用户调用接口关系表
create table if not exists sonapi.`user_interface_info`
(
    `id` bigint not null auto_increment comment '主键' primary key,
...
insert into sonapi.`interface_info` (`name`, `description`, `url`, `requestHeader`, `responseHeader`, `status`,
                                       `method`, `userId`)
